...
OUTPUT = MiggyOS/boot/kernel.bin

run: all
	printf "\n$(EMULATOR) $(EMULATOR_FLAGS) $(OUTPUT)" > OS
	chmod +x OS
all:$(OBJS)
	mkdir MiggyOS/ -p
	mkdir MiggyOS/boot/ -p
...
build:all
	...
	rm MiggyOS/boot/grub/ -r -f
	mkdir MiggyOS/boot/grub/
	echo set default=0 >> MiggyOS/boot/grub/grub.cfg
	echo set timeout=0 >> MiggyOS/boot/grub/grub.cfg
	echo menuentry "MiggyOS" { >> MiggyOS/boot/grub/grub.cfg
	echo         set root='(hd96)' >> MiggyOS/boot/grub/grub.cfg
	echo         multiboot /boot/kernel.bin >> MiggyOS/boot/grub/grub.cfg
	echo } >> MiggyOS/boot/grub/grub.cfg

	grub-mkrescue -o MiggyOS.iso MiggyOS/
